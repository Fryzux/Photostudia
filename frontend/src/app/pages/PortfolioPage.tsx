...
export function PortfolioPage() {
  return (
    <div className="space-y-6 sm:space-y-8">
      <section data-reveal="section" className="reveal-section mono-panel rounded-[2rem] border border-[#111111]/8 px-5 py-10 text-center sm:px-8 sm:py-12">
          ...
          <CarouselContent>
            {portfolioItems.map((item, index) => (
              <CarouselItem key={item.id} className="lg:basis-1/2">
                <article className="reveal-card mono-panel lift-card overflow-hidden rounded-[1.9rem] border border-[#111111]/8 text-center" style={{ transitionDelay: `${100 + index * 70}ms` }}>
